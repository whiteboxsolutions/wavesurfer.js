  ...
  ...glob
    ...
    .map((plugin) => [
      // ES module
      {
        input: plugin,
        output: {
          file: plugin.replace('src/plugins', 'dist/plugins').replace('.ts', '.js'),
          format: 'esm',
        },
        plugins,
      },
      // ES module again but with an .esm.js extension
      {
        input: plugin,
        output: {
          file: plugin.replace('src/plugins', 'dist/plugins').replace('.ts', '.esm.js'),
          format: 'esm',
        },
        plugins,
      },
      // CommonJS module (Node.js)
      {
        ...
        output: {
          ...
          file: plugin.replace('src/plugins', 'dist/plugins').replace('.ts', '.cjs'),
          format: 'cjs',
          exports: 'default',
        },
        plugins,
      ...
      // UMD (browser script tag)
      {
        ...
        output: {
          ...
          file: plugin.replace('src/plugins', 'dist/plugins').replace('.ts', '.min.js'),
          format: 'umd',
          extend: true,
          globals: {
